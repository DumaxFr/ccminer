...
#include "./cuda_b_blake512.h"

#define BLAKE512_TPB80 512
#define BLAKE512_TPB64 192
#define BLAKE512_TPB64F 192
...
#define GS4(a,b,c,d,e,f,a1,b1,c1,d1,e1,f1,a2,b2,c2,d2,e2,f2,a3,b3,c3,d3,e3,f3){\
	...
	v[ d] = SWAPUINT2(v[ d] ^ v[ a]);	v[d1] = SWAPUINT2(v[d1] ^ v[a1]);	v[d2] = SWAPUINT2(v[d2] ^ v[a2]);	v[d3] = SWAPUINT2(v[d3] ^ v[a3]);\
...
void cuda_base_blake512_cpu_setBlock_80(uint32_t *endiandata) {
    ...
    CUDA_SAFE_CALL(cudaMemcpyToSymbol(c_v, v, sizeof(v), 0, cudaMemcpyHostToDevice));
...
__global__
__launch_bounds__(BLAKE512_TPB80, 2)
void cuda_base_blake512_gpu_hash_80(const uint32_t threads, const uint32_t startNounce, uint2x4 *const __restrict__ g_hash) {
...
void cuda_base_blake512_cpu_hash_80(const uint32_t threads, const uint32_t startNounce, uint32_t *d_outputHash) {
    dim3 grid((threads + BLAKE512_TPB80 - 1) / BLAKE512_TPB80);
    dim3 block(BLAKE512_TPB80);

    cuda_base_blake512_gpu_hash_80<<<grid, block>>>(threads, startNounce, (uint2x4*)d_outputHash);
}

#pragma endregion

#pragma region Blake512_64

__global__
__launch_bounds__(BLAKE512_TPB64, 2)
void cuda_base_blake512_gpu_hash_64(const uint32_t threads, uint2x4* g_hash) {
...
__host__
void cuda_base_blake512_cpu_hash_64(const uint32_t threads, uint32_t *d_hash) {

    uint32_t tpb = BLAKE512_TPB64;

    const dim3 grid((threads + tpb - 1) / tpb);
    const dim3 block(tpb);

    cuda_base_blake512_gpu_hash_64<<<grid, block>>>(threads, (uint2x4*)d_hash);
}

#pragma endregion

#pragma region Blake512_64_final

__global__
__launch_bounds__(BLAKE512_TPB64F, 2)
void cuda_base_blake512_gpu_hash_64f(const uint32_t threads, const uint2x4* __restrict__ g_hash, const uint32_t startNonce, uint32_t *resNonce, const uint64_t target) {
    ...
    if (thread < threads) {
        ...
        const uint2x4 *phash = &g_hash[thread << 1];
        uint2x4 *outpt = (uint2x4*)msg;
        outpt[0] = __ldg4(&phash[0]);
        outpt[1] = __ldg4(&phash[1]);
        ...
        GS4(0, 4, 8, 12, 2, 12, 1, 5, 9, 13, 6, 10, 2, 6, 10, 14, 0, 11, 3, 7, 11, 15, 8, 3);
        //GS4(0, 5, 10, 15, 4, 13,	1, 6, 11, 12, 7, 5,		2, 7, 8, 13, 15, 14,		3, 4, 9, 14, 1, 9);
        v[1] += (m[7] ^ z[5]) + v[6]; // 1,3,4,6,9,11,12,14
        v[3] += (m[1] ^ z[9]) + v[4]; // 1,3,4,6,9,11,12,14

        v[12] = SWAPUINT2(v[12] ^ v[1]); // 1,3,4,6,9,11,12,14
        v[14] = SWAPUINT2(v[14] ^ v[3]); // 1,3,4,6,9,11,12,14

        v[11] += v[12]; // 1,3,4,6,9,11,12,14
        v[9] += v[14]; // 1,3,4,6,9,11,12,14

        v[6] = ROR2(v[6] ^ v[11], 25); // 1,3,4,6,9,11,12
        v[4] = ROR2(v[4] ^ v[9], 25); // 1,3,4,6,9,11,12

        v[1] += (m[5] ^ z[7]) + v[6]; // 1,3,4,6,11,12
        v[3] += (m[9] ^ z[1]) + v[4]; // 1,3,4,11,12

        v[12] = ROR16(v[12] ^ v[1]); // 1,3,11,12
        v[11] += v[12]; // 3,11,12

        v[3] = cuda_swab64_U2(xor3x(v[3], h[3], v[11])); // 3,11

        uint64_t check = *(uint64_t*)&v[3];
        if (check <= target) {
...
void cuda_base_blake512_cpu_hash_64f(const uint32_t threads, const uint32_t* d_outputHash, const uint32_t startNonce, uint32_t *d_resNonce, const uint64_t target) {
    uint32_t tpb = BLAKE512_TPB64F;

    //int dev_id = device_map[thr_id];
    //if (device_sm[dev_id] <= 500) tpb = BLAKE512_TPB64_LEGACY;

    const dim3 grid((threads + tpb - 1) / tpb);
    const dim3 block(tpb);

    cuda_base_blake512_gpu_hash_64f<<<grid, block>>>(threads, (uint2x4*)d_outputHash, startNonce, d_resNonce, target);
}

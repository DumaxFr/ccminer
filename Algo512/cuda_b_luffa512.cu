...
#include "./cuda_b_luffa512.h"

#define LUFFA512_TPB80 256
#define LUFFA512_TPB64 352
#define LUFFA512_TPB64F 352
...
__device__
static void rnd512_64_first(uint32_t *const __restrict__ statebuffer, uint32_t *const __restrict__ statechainv) {
	...
	for (int j = 0; j<5; j++) {
		...
		#pragma unroll 8
		for(int i=0;i<8;i++)
			statechainv[i+(j<<3)] ^= statebuffer[i];
		MULT0(statebuffer);
	}
...
__device__
static void rnd512_80_first(uint32_t *const __restrict__ statebuffer, uint32_t *const __restrict__ statechainv) {
...
__global__
__launch_bounds__(LUFFA512_TPB80, 4)
void cuda_base_luffa512_gpu_hash_80(const uint32_t threads, const uint32_t startNounce, uint32_t *outputHash)
{
	...
	if (thread < threads)
	{
		...
		*(uint4*)&statebuffer[ 4] = *(uint4*)&statebufferpre[ 4];
		...
		rnd512_80_first(statebuffer, statechainv);
		...
		#pragma unroll 8
		for(int i=0;i<8;i++)
			outHash[i] = cuda_swab32(statechainv[i] ^ statechainv[i+8] ^ statechainv[i+16] ^ statechainv[i+24] ^ statechainv[i+32]);

		rnd512_nullhash(statechainv);

		#pragma unroll 8
		for(int i=0;i<8;i++)
			outHash[8+i] = cuda_swab32(statechainv[i] ^ statechainv[i+8] ^ statechainv[i+16] ^ statechainv[i+24] ^ statechainv[i+32]);

	}
}

__host__
void cuda_base_luffa512_cpu_hash_80(const uint32_t threads, const uint32_t startNounce, uint32_t *d_outputHash){

	dim3 grid((threads + LUFFA512_TPB80-1)/LUFFA512_TPB80);
	dim3 block(LUFFA512_TPB80);

	cuda_base_luffa512_gpu_hash_80 <<<grid, block>>> (threads, startNounce, d_outputHash);
}

__host__
void luffa512_cpu_precalc() {
	uint32_t tmp,i,j;
	uint32_t statebuffer[8];
	uint32_t t[40];
	uint32_t statechainv[40] =
...
__host__
void cuda_base_luffa512_setBlock_80(void *pdata)
{
	memcpy(PaddedMessage, pdata, 80);

	CUDA_SAFE_CALL(cudaMemcpyToSymbol( c_PaddedMessage80, PaddedMessage, 10*sizeof(uint64_t), 0, cudaMemcpyHostToDevice));
	luffa512_cpu_precalc();
}

#pragma endregion

#pragma region Luffa512_64

__global__
__launch_bounds__(LUFFA512_TPB64, 1)
void cuda_base_luffa512_gpu_hash_64(const uint32_t threads, uint32_t *g_hash){
	...
	if (thread < threads) {
		...
		rnd512_64_first(statebuffer, statechainv);
        ...
        #pragma unroll
        for (int i = 0; i < 16; i+=4)
            AS_UINT4(&pHash[i]) = AS_UINT4(&hash[i]);
	}
}


__host__
void cuda_base_luffa512_cpu_hash_64(const uint32_t threads, uint32_t *d_hash) {

    dim3 grid((threads + LUFFA512_TPB64-1)/LUFFA512_TPB64);
    dim3 block(LUFFA512_TPB64);

    cuda_base_luffa512_gpu_hash_64<<<grid, block>>>(threads, d_hash);
}

#pragma endregion

#pragma region Luffa512_64_final

__global__
__launch_bounds__(LUFFA512_TPB64F, 1)
void cuda_base_luffa512_gpu_hash_64f(const uint32_t threads, const uint32_t* __restrict__ g_hash, const uint32_t startNonce, uint32_t* resNonce, const uint64_t target) {
	...
	if (thread < threads) {
		...
		hash[6] = cuda_swab32(statechainv[6] ^ statechainv[14] ^ statechainv[22] ^ statechainv[30] ^ statechainv[38]);
		hash[7] = cuda_swab32(statechainv[7] ^ statechainv[15] ^ statechainv[23] ^ statechainv[31] ^ statechainv[39]);

        uint64_t check = *(uint64_t*)&hash[6];
		if (check <= target) {
...
__host__
void cuda_base_luffa512_cpu_hash_64f(const uint32_t threads, const uint32_t* d_hash, const uint32_t startNonce, uint32_t* d_resNonce, const uint64_t target) {

    dim3 grid((threads + LUFFA512_TPB64F-1)/LUFFA512_TPB64F);
    dim3 block(LUFFA512_TPB64F);

    cuda_base_luffa512_gpu_hash_64f<<<grid, block>>>(threads, d_hash, startNonce, d_resNonce, target);
}

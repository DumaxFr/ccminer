...
#define CONST_EXP3d(i)   devectorize(ROL2(q[i+ 1], 5))    + devectorize(ROL2(q[i+ 3],11)) + devectorize(ROL2(q[i+5], 27)) + \
                         devectorize(SWAPUINT2(q[i+7])) + devectorize(ROL2(q[i+9], 37)) + devectorize(ROL2(q[i+11],43)) + \
...
void cuda_base_bmw512_gpu_hash_64(const uint32_t threads, uint64_t *g_hash) {
	...
	if (thread < threads) {
        ...
        #pragma unroll
        for (int i = 0; i < 8; i+=2) {
            AS_UINT4(&msg[i]) = AS_UINT4(&pHash[i]);
        }
        ...
        const uint64_t hash2[16] = {
	        ...
	        0xE0E1E2E3E4E5E6E7, 0xE8E9EAEBECEDEEEF, 0xF0F1F2F3F4F5F6F7, 0xF8F9FAFBFCFDFEFF ^ 512
        };
        ...
        tmp = vectorize(mxh[3] - mxh[2] + hash2[10] - hash2[12] + hash2[15]);
        ...
        tmp = vectorize(mxh[2] - mxh[5] - mxh[6] + hash2[13] - hash2[15]);
        ...
        tmp = vectorize(hash2[15] + hash2[8] - mxh[1] - mxh[4] - mxh[7]);
        ...
        q[17] += (SHR2(q[15], 2) ^ SHL2(q[15], 2) ^ ROL2(q[15], 28) ^ ROL2(q[15], 59)) + (SHR2(q[16], 1) ^ SHL2(q[16], 3) ^ ROL2(q[16], 4) ^ ROL2(q[16], 37))
        ...
        h[1] = (SHR2(XH64, 7) ^ SHL2(q[17], 8) ^ vectorize(msg[1])) + (XL64 ^ q[25] ^ q[1]);
        ...
        h[8] = (ROL2(h[4], 9)) + (XH64 ^ q[24] ^ 0x80) + (SHL2(XL64, 8) ^ q[23] ^ q[8]);
        ...
        h[15] = (ROL2(h[3], 16)) + (XH64 ^ q[31] ^ 512) + (SHR2(XL64, 2) ^ q[22] ^ q[15]);
		...
		add1 = devectorize(q[2] + q[4] + q[6] + q[8] + q[10] + q[12] + q[14]);
		add2 = devectorize(q[3] + q[5] + q[7] + q[9] + q[11] + q[13] + q[15]);
		...
		add1 -= devectorize(q[2] - q[16]);
		add2 -= devectorize(q[3] - q[17]);

		XL64 = XL64 ^ q[18] ^ q[19];

		q[20] = vectorize(add1 + CONST_EXP3d(4) + devectorize((precalc[4] + ROL2(h[4], 5) + ROL8(h[7]) - ROL2(h[14], 15)) ^ cmsg[11]));
		q[21] = vectorize(add2 + CONST_EXP3d(5) + devectorize((precalc[5] + ROL2(h[5], 6) + ROL2(h[8], 9) - ROL2(h[15], 16)) ^ cmsg[12]));

		add1 -= devectorize(q[4] - q[18]);
		add2 -= devectorize(q[5] - q[19]);
		...
		q[28] = vectorize(add1 + CONST_EXP3d(12) + devectorize((precalc[12] + ROL2(h[12], 13) + ROL2(h[15], 16) - ROL2(h[6], 7)) ^ cmsg[3]));
		...
		q[31] = vectorize(add2 + CONST_EXP3d(15) + devectorize((precalc[15] + ROL2(h[15], 16) + ROL2(h[2], 3) - ROL2(h[9], 10)) ^ cmsg[6]));

		XH64 = XH64 ^ q[30] ^ q[31];

		msg[0] = devectorize((SHL2(XH64, 5) ^ SHR2(q[16], 5) ^ h[0]) + (XL64 ^ q[24] ^ q[0]));
		msg[1] = devectorize((SHR2(XH64, 7) ^ SHL2(q[17], 8) ^ h[1]) + (XL64 ^ q[25] ^ q[1]));
		...
		msg[8] = devectorize((XH64 ^ q[24] ^ h[8]) + (SHL2(XL64, 8) ^ q[23] ^ q[8]) + ROTL64(msg[4], 9));
       ...
       #pragma unroll
        for (int i = 0; i < 8; i+=2) {
            AS_UINT4(&pHash[i]) = AS_UINT4(&msg[i+8]);
        }
	}
}
...
#pragma region BMW512_64_final

__global__
__launch_bounds__(BMW512_TPB64F, 2)
void cuda_base_bmw512_gpu_hash_64f(const uint32_t threads, const uint64_t* __restrict__ g_hash, const uint32_t startNonce, uint32_t *resNonce, const uint64_t target) {

	uint32_t thread = (blockDim.x * blockIdx.x + threadIdx.x);
	if (thread < threads) {

        const uint64_t *pHash = &g_hash[thread * 8];
		...
		uint64_t chk = devectorize((SHR2(XH64, 11) ^ SHL2(q[23], 2) ^ h[7]) + (XL64 ^ q[31] ^ q[7]));
        uint64_t check = devectorize((XH64 ^ q[27] ^ h[11]) + (SHL2(XL64, 4) ^ q[18] ^ q[11]) + ROTL64(chk, 12));
...
void cuda_base_bmw512_cpu_hash_64f(const uint32_t threads, const uint32_t *d_hash, const uint32_t startNonce, uint32_t *d_resNonce, const uint64_t target) {
	...
	const uint32_t threadsperblock = BMW512_TPB64F;
	dim3 grid((threads + threadsperblock-1)/threadsperblock);
	dim3 block(threadsperblock);

	cuda_base_bmw512_gpu_hash_64f<<<grid, block>>>(threads, (uint64_t*)d_hash, startNonce, d_resNonce, target);
}

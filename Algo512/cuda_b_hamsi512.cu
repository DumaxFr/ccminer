...
#pragma region Hamsi512_80

__constant__ static uint32_t __align__(64) precalc_c[17];
__constant__ static uint32_t __align__(64) precalc_h[16];

__host__
void cuda_base_hamsi512_setBlock_80(void *pdata) {

	unsigned char h1[72];

    for (int i = 0; i < 9; i++) {
        ((uint2*)h1)[i] = ((uint2*)pdata)[i];
    }
	...
	cudaMemcpyToSymbol(precalc_h, h, sizeof(precalc_h), 0, cudaMemcpyHostToDevice);
	cudaMemcpyToSymbol(precalc_c, c, sizeof(precalc_c), 0, cudaMemcpyHostToDevice);

}

__global__
__launch_bounds__(HAMSI512_TPB80, 4)
void cuda_base_hamsi512_gpu_hash_80(const uint32_t threads, const uint32_t startNonce, uint32_t *g_hash) {

    uint32_t thread = (blockDim.x * blockIdx.x + threadIdx.x);

    if (thread < threads) {
		uint32_t h[16] = { precalc_h[0], precalc_h[1], precalc_h[2], precalc_h[3], precalc_h[4], precalc_h[5], precalc_h[6], precalc_h[7], precalc_h[8], precalc_h[9], precalc_h[10], precalc_h[11], precalc_h[12], precalc_h[13], precalc_h[14], precalc_h[15] };

		uint32_t c0 = precalc_c[0], c1 = precalc_c[1], c2 = precalc_c[2], c3 = precalc_c[3];
		uint32_t c4 = precalc_c[4], c5 = precalc_c[5], c6 = precalc_c[6], c7 = precalc_c[7];
		uint32_t c8 = precalc_c[8], c9 = precalc_c[9], cA = precalc_c[10], cB = precalc_c[11];
		uint32_t cC = precalc_c[12], cD = precalc_c[13], cE = precalc_c[14], cF = precalc_c[15];

		uint32_t m0, m1, m2, m3, m4, m5, m6, m7, m8, m9, mA, mB, mC, mD, mE, mF;
		uint32_t *tp, db, dm;

        unsigned char h1[8];
		((uint32_t*)h1)[1] = cuda_swab32(startNonce + thread);
		((uint32_t*)h1)[0] = precalc_c[16];
		...
		for (int i = 0; i < 16; i+=4)
            AS_UINT4(&Hash[i]) = make_uint4(cuda_swab32(h[i]), cuda_swab32(h[i+1]), cuda_swab32(h[i+2]), cuda_swab32(h[i+3]));
...
void cuda_base_hamsi512_cpu_hash_80(const uint32_t threads, const uint32_t startNonce, uint32_t *d_hash) {
    const uint32_t threadsperblock = HAMSI512_TPB80;

    dim3 grid((threads + threadsperblock - 1) / threadsperblock);
    dim3 block(threadsperblock);

    cuda_base_hamsi512_gpu_hash_80 <<<grid, block>>> (threads, startNonce, d_hash);
}

#pragma endregion

#pragma region Hamsi512_64

__global__
__launch_bounds__(HAMSI12_TPB64, 4)
void cuda_base_hamsi512_gpu_hash_64(const uint32_t threads, uint32_t *g_hash) {
    ...
    if (thread < threads) {
        ...
        asm("prefetch.global.L1 [%0];" : : "l"(pHash));
        ...
        for (int i = 0; i < 64; ) {
            ...
            asm("ld.global.v2.u32 {%0, %1}, [%2];"
                : "=r"(preload[0]), "=r"(preload[1])
                : "l"(&pHash[i>>2]) );

            do {
                db = ((unsigned char *)&preload[0])[i % 8];
                //#pragma unroll 2
                for (int v = 0; v < 8; v++, db >>= 1) {
                    dm = -(uint32_t)(db & 1);
                    m0 ^= dm & *(tp + 0); m1 ^= dm & *(tp + 1);
        ...
        for (int i = 0; i < 16; i += 2) {
            uint32_t outH[2];
            outH[0] = cuda_swab32(h[i]);
            outH[1] = cuda_swab32(h[i + 1]);
            AS_UINT2(&pHash[i]) = AS_UINT2(outH);
        }
...
void cuda_base_hamsi512_cpu_hash_64(const uint32_t threads, uint32_t *d_hash) {
    const uint32_t threadsperblock = HAMSI12_TPB64;

    dim3 grid((threads + threadsperblock - 1) / threadsperblock);
    dim3 block(threadsperblock);

    cuda_base_hamsi512_gpu_hash_64 <<<grid, block>>> (threads, d_hash);
}

#pragma endregion

#pragma region Hamsi512_64_final

__global__
__launch_bounds__(HAMSI12_TPB64F, 4)
void cuda_base_hamsi512_gpu_hash_64f(const uint32_t threads, const uint32_t* __restrict__ g_hash, const uint32_t startNonce, uint32_t *resNonce, const uint64_t target) {
    ...
    if (thread < threads) {
        ...
        uint32_t outH[2];
        outH[0] = cuda_swab32(h[6]);
        outH[1] = cuda_swab32(h[7]);
        uint64_t check = *(uint64_t*)&outH[0];
		if (check <= target) {
...
void cuda_base_hamsi512_cpu_hash_64f(const uint32_t threads, const uint32_t * d_hash, const uint32_t startNonce, uint32_t * d_resNonce, const uint64_t target) {
    const uint32_t threadsperblock = HAMSI12_TPB64F;

    dim3 grid((threads + threadsperblock - 1) / threadsperblock);
    dim3 block(threadsperblock);

    cuda_base_hamsi512_gpu_hash_64f <<<grid, block>>> (threads, d_hash, startNonce, d_resNonce, target);
}

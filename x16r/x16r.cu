...
#define _PROFILE_METRICS_X16R

#ifdef _PROFILE_METRICS_X16R
...
extern "C" int scanhash_x16x(int thr_id, struct work* work, uint32_t max_nonce, unsigned long *hashes_done, const char variation) {
    ...
    if (opt_benchmark) {
        ...
        ((uint8_t*)pdata)[8] = 0xAA; // hashOrder[0] = 'A'; for echo 80 + echo 64
    ...
    switch (algo80) {
        ...
        case SIMD:
            ...
            break;
        case ECHO:
            echo512_setBlock_80((void*)endiandata);
            break;
        case HAMSI:
    ...
    do {
        ...
        switch (algo80) {
            ...
            case SIMD:
                ...
                break;
            case ECHO:
                echo512_cuda_hash_80(throughput, pdata[19], d_hash[thr_id]); order++;
                TRACE("echo   :");
                break;
            case HAMSI:
        ...
        if (opt_debug) {
            ...
            avgDuration80[algo80][thr_id] += (milliseconds - avgDuration80[algo80][thr_id]) / (float)(totalRuns80[algo80][thr_id] + 1);
            totalRuns80[algo80][thr_id]++;
        }
        #endif // _PROFILE_METRICS_X16R
        ...
        for (int i = 1; i < 16; i++) {
            ...
            if (opt_debug) {
                ...
                cudaEventDestroy(x16r_kernel_start[thr_id]);
                cudaEventDestroy(x16r_kernel_stop[thr_id]);
                avgDuration64[algo64][thr_id] += (milliseconds - avgDuration64[algo64][thr_id]) / (float)(totalRuns64[algo64][thr_id] + 1);
                totalRuns64[algo64][thr_id]++;
            }
            #endif // _PROFILE_METRICS_X16R
        }
    ...
    if (opt_debug) {
        for (int i = 0; i < HASH_FUNC_COUNT; i++) {
            gpulog(LOG_CRITICAL, thr_id, "%s-80 AvgDuration after %d runs : %f ms",
                algo_strings[i], totalRuns80[i][thr_id], avgDuration80[i][thr_id]);
            gpulog(LOG_CRITICAL, thr_id, "%s-64 AvgDuration after %d runs : %f ms",
                algo_strings[i], totalRuns64[i][thr_id], avgDuration64[i][thr_id]);
        }
    }
    #endif // _PROFILE_METRICS_X16R

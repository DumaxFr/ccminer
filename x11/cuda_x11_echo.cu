...
void cuda_echo_round(
    ...
    #ifdef CCMINER_REFERENCE
    for (int k = 1; k < 10; k++)
        #else
    for (int k = 1; k < 9; k++)
        #endif // CCMINER_REFERENCE
    {
    ...
    #ifndef CCMINER_REFERENCE
    {
        ...
        AES_2ROUND(sharedMemory, W[0], W[1], W[2], W[3], k0); // W0
        k0++;
        AES_2ROUND(sharedMemory, W[8], W[9], W[10], W[11], k0); // W2
        k0 += 2;
        AES_2ROUND(sharedMemory, W[20], W[21], W[22], W[23], k0); // W5
        k0++;
        AES_2ROUND(sharedMemory, W[28], W[29], W[30], W[31], k0); // W7
        AES_2ROUND(sharedMemory, W[32], W[33], W[34], W[35], k0); // W8
        k0++;
        AES_2ROUND(sharedMemory, W[40], W[41], W[42], W[43], k0); // W10
        k0 += 2;
        AES_2ROUND(sharedMemory, W[52], W[53], W[54], W[55], k0); // W13
        k0++;
        AES_2ROUND(sharedMemory, W[60], W[61], W[62], W[63], k0); // W15
        ...
        {
            #pragma unroll 2
            for (int idx = 0; idx < 64; idx += 32) // Schleife über die elemnte
            {
                uint32_t a = W[idx + i];
                uint32_t b = W[idx + i + 4];
                uint32_t c = W[idx + i + 8];
...
void x11_echo512_gpu_hash_64(uint32_t threads, uint32_t startNounce, uint64_t *g_hash, uint32_t *g_nonceVector) {
    ...
    if (thread < threads) {
        int hashPosition = (g_nonceVector != NULL) ? g_nonceVector[thread] - startNounce : thread;

...
void aes_gpu_init_mt_256(uint32_t sharedMemory[4][256]) {
	...
	if(threadIdx.x<256) {
		...
		sharedMemory[1][threadIdx.x] = CU32_ROL8(temp);
		sharedMemory[2][threadIdx.x] = CU32_ROL16(temp);
		sharedMemory[3][threadIdx.x] = CU32_ROR8(temp);
	}
}
...
void aes_gpu_init_lt_256(uint32_t sharedMemory[4][256]) {
	...
	if(threadIdx.x<128) {
		...
		sharedMemory[1][(threadIdx.x<<1) + 0] = CU32_ROL8(temp.x);
		sharedMemory[1][(threadIdx.x<<1) + 1] = CU32_ROL8(temp.y);
		sharedMemory[2][(threadIdx.x<<1) + 0] = CU32_ROL16(temp.x);
		sharedMemory[2][(threadIdx.x<<1) + 1] = CU32_ROL16(temp.y);
		sharedMemory[3][(threadIdx.x<<1) + 0] = CU32_ROR8(temp.x);
		sharedMemory[3][(threadIdx.x<<1) + 1] = CU32_ROR8(temp.y);
	}
}
...
void aes_gpu_init64(uint32_t sharedMemory[4][256]) {
	/* each thread startup will fill 4 uint32 */
	uint4 temp = __ldg(&((uint4*)&d_AES0)[threadIdx.x]);
	
	sharedMemory[0][(threadIdx.x<<2) + 0] = temp.x;
	sharedMemory[0][(threadIdx.x<<2) + 1] = temp.y;
	sharedMemory[0][(threadIdx.x<<2) + 2] = temp.z;
	sharedMemory[0][(threadIdx.x<<2) + 3] = temp.w;

	sharedMemory[1][(threadIdx.x<<2) + 0] = CU32_ROL8(temp.x);
	sharedMemory[1][(threadIdx.x<<2) + 1] = CU32_ROL8(temp.y);
	sharedMemory[1][(threadIdx.x<<2) + 2] = CU32_ROL8(temp.z);
	sharedMemory[1][(threadIdx.x<<2) + 3] = CU32_ROL8(temp.w);

    sharedMemory[2][(threadIdx.x<<2) + 0] = CU32_ROL16(temp.x);
	sharedMemory[2][(threadIdx.x<<2) + 1] = CU32_ROL16(temp.y);
	sharedMemory[2][(threadIdx.x<<2) + 2] = CU32_ROL16(temp.z);
	sharedMemory[2][(threadIdx.x<<2) + 3] = CU32_ROL16(temp.w);

	sharedMemory[3][(threadIdx.x<<2) + 0] = CU32_ROR8(temp.x);
	sharedMemory[3][(threadIdx.x<<2) + 1] = CU32_ROR8(temp.y);
	sharedMemory[3][(threadIdx.x<<2) + 2] = CU32_ROR8(temp.z);
	sharedMemory[3][(threadIdx.x<<2) + 3] = CU32_ROR8(temp.w);
}
...
__device__ __forceinline__
static void aes_round2_cripple_23(const uint32_t sharedMemory[4][256],const uint32_t x0,const uint32_t x1,const uint32_t x2,const uint32_t x3, uint32_t &y2, uint32_t &y3) {
